...
def get_many_faces(vision_frames: List[VisionFrame]) -> List[Face]:
	...
	total_frames = len(vision_frames)
	from facefusion import logger
	
	for index, vision_frame in enumerate(vision_frames):
		# Log frame progress only occasionally to reduce noise
		if index % 10 == 0 or index == total_frames - 1:
			logger.info("face_analyser", f"Processing frame {index+1}/{total_frames} - {((index+1)/total_frames*100):.1f}%")
		...
		for angle in angles_to_try:
			logger.debug("face_analyser", f"Trying angle {angle}")
			...
			if bounding_boxes and face_scores and face_landmarks_5 and state_manager.get_item('face_detector_score') > 0:
				...
				if faces:
					...
					logger.debug("face_analyser", f"found {len(faces)} faces at angle {angle}")
					faces_found = True
					break
		
		if not faces_found and index % 10 == 0:
			logger.debug("face_analyser", f"No faces detected in frame {index} after trying all angles")

	logger.info("face_analyser", f"Face detection complete: found faces in {len(many_faces)} instances")
	return many_faces
